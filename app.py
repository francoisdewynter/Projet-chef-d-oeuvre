...
img=Image.open('Round-Pure-Brilliance-sw-zirconia.jpg')
st.image(img,width=200)


def create_usertable():
    c.execute('DROP TABLE IF EXISTS users')
    c.execute('CREATE TABLE IF NOT EXISTS users(id INTEGER PRIMARY KEY AUTOINCREMENT, admin INTEGER, username TEXT, password TEXT)')
    conn.commit()
...
def create_resultstable():
    ...
    c.execute('CREATE TABLE IF NOT EXISTS results(id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT, predictions REAL)')
    conn.commit()
    ...
    if st.sidebar.checkbox('Login'):
        ...
        if result:
            ...
            if task == 'Zirconia database':
                ...
                img1=Image.open('c0014142-800px-wm.jpg')
                st.image(img1,width=500)
                st.subheader('Price Prediction')
                st.write('Quality of zirconia')
                cut=st.slider('Cut', min_value=1, max_value=6)
                color=st.slider('Color', min_value=1, max_value=10)
                clarity=st.slider('Clarity', min_value=1, max_value=9)
                st.write('Dimensions of zirconia')
                carat= st.slider('Carat', min_value=0.0, max_value=5.0)
                depth= st.slider('Depth', min_value=50.0, max_value=73.0)
                table=st.slider('Table', min_value=49.0, max_value=79.0)
                width=st.slider('Width', min_value=3.0, max_value=10.0)
                height=st.slider('Height', min_value=1.0, max_value=4.0)
    ...
    if autorisation=="Yes":
        ...
        if password!="YoungLink93@*":
            ...
            if st.button("Sign up") and password3==password4:
                ...
                #create_usertable()
                add_userdata(admin, username, password4)
